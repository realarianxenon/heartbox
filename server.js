...
if (existing.c === 0) {
  ...
  console.log('✅ داروهای پیش‌فرض افزوده شدند.');
}
...
app.use(session({
  ...
  cookie: { maxAge: 7 * 24 * 60 * 60 * 1000 } // 7 روز
}));
...
app.post('/api/auth/register', (req, res) => {
  ...
  const role = role_request === 'doctor' || role_request === 'pharmacist' ? 'patient' : 'patient';
...
app.post('/api/appointments', (req, res) => {
  ...
  // بررسی تداخل (برای پزشک مشخص در یک ساعت مشخص)
...
// ===== 🏠 صفحه اصلی (برای مسیرهای ناشناخته) =====
app.get('*', (req, res) => {
...
// ===== راه‌اندازی سرور =====
app.listen(PORT, () => {
  console.log(`🚀 هارت‌باکس روی http://localhost:${PORT} اجرا شد!`);
});